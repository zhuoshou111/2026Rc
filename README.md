...
当前源码基线为固件 `v153`。正式入口已经核对并清理：上电只进行安全 PWM、陀螺仪、升降 GPIO、底盘、串口和 PE6 输入初始化，不包含上电自动升降、自动转台、自动抓放、自动移动或一次性参数候选分支。mode 80/81/82/83 是调参工作台长期使用的受限试动接口，mode 126 是保留的陀螺仪诊断接口，均不属于上电自动测试；mode 89 启用比赛启动后会锁定三路舵机试动。
...
- 任务1、任务2动作试跑；
- “任务试跑”页可滚动查看全部动作按钮；
- mode 21/22 长方体抓取试跑（预闭合 + 转台摇摆后放到舵盘2/4）；
- 任务1抓取/放置按舵盘1~5、任务2抓取/放置按奖杯1~3（冠/亚/季）各自独立调时序；长方体抓取仍有共享时序（调参台新增“任务时序（二级）”页，先选动作再调每步等待）；
...
当前 `v153` 源码已于 `2026-09-01` 通过 Keil 编译，结果为 `0 Error(s), 0 Warning(s)`（新增 mode 21/22 长方体抓取，抓取/放置时序按舵盘与奖杯分别独立调参）。此前的 `v149` 构建产物 `OBJ/chassis_hardware_tuning_v149_20260829_171302.bin` 仍是最后一次确认的 BIN 基线；本次只编译了源码，尚未烧录。
...
mode 21/22（`RPI_CMD_GRUB_CUBOID_DISC2` / `RPI_CMD_GRUB_CUBOID_DISC4`）用于从地面抓取长方体物块：升降下降到地面抓取高度后，夹爪先预闭合（`SERVO2_CUBOID_PRE_CLOSE_PULSE_US`），随后底部旋转舵机在 24° 附近快速小范围摇摆两次（24→0→24→24→48→24），确保物块正对夹爪后再完全夹紧，随后分别把物块放到舵盘2 / 舵盘4。摇摆的 0°/48° 脉宽和步进等待分别由 `SERVO3_CUBOID_WIGGLE_LOW_PULSE_US`、`SERVO3_CUBOID_WIGGLE_HIGH_PULSE_US`、`SERVO3_CUBOID_WIGGLE_STEP_MS` 控制，均可按机械实测调整。

抓取/放置任务的每个动作等待时间已独立拆分，默认值与原共享等待一致（夹爪 380 ms、转台 850 ms、保持 200 ms、回正后张爪 500 ms、地面分段张开 100 ms）。任务1的抓取（mode 7/8/9/10/12 → 舵盘1~5）和放置（mode 14~18 ← 舵盘1~5）各自独立；任务2的抓取（mode 42/43/44 → 舵盘1/3/5）和领奖台放置（mode 33/34/35 → 冠/亚/季）也各自独立；长方体抓取（mode 21/22）目前使用一组共享时序。调参台新增“任务时序（二级）”页：先点动作按钮（如“任务2 放置2 舵盘3→亚军”），再对该动作的每步等待单独微调，改完到“保存 / 编译 / 烧录”页写回源码，并用“任务试跑”页对应 mode 实测。`SERVO1/2/3_SETTLE_MS` 仍用于手动单项试动，`ARM_ACTION_HOLD_MS`、`SERVO3_HOME_BEFORE_CLAW_OPEN_MS`、`GROUND_RELEASE_STAGE_INTERVAL_MS` 保留为历史默认值参考。
